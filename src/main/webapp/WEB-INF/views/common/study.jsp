...
<body>
	...
	<pre>
		...
		3. 브라우저는 응답받은 HTML데이터를 처음부터 끝까지 렌더링 
			-> 전체페이지를 다시 로딩 -> 화면이 한번 깜빡(SSR)
		
		비동기방식 :
		1. 사용자가 요청을 보냄
		2. JavaScript기술을 이용해서 데이터만 서버로 띡 전송
		3. 서버는 JSON / XML데이터만 응답 
		4. JavaScript를 이용해서 필요한 부부만 갱신 -> 부드럽고, 빠름 (CSR)
	...
	<pre>
		...
		사람이 읽기 쉽고, 기계가 파싱하기 쉬운 데이터 교환형식 텍스트기반이라 아주 가볍다ㅇ
		...
		function fn1() {
			...
			$.ajax({
				...
				error : result => {
					...
					document.querySelector('#output1').innerHTML = '통신에 실패했습니다 ~~~ㅠ';
				},
				complete : () => {
